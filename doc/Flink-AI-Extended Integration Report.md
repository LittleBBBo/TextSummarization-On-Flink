...
#### \[Issue-10][Flink-AI-Extended] Simplify the ExampleCoding configuration process

see issue [@link](https://github.com/alibaba/flink-ai-extended/issues/17)

现在配置ExampleCoding的过程较为繁琐，作为一种常用的配置，我觉得可以添加一些工具接口来帮助用户简单地进行配置。
...
可见，用户配置时需要了解字段的名称、类型、各种常量等，其实均可以将其封装起来，只需要用户提供输入和输出table的schema即可完成配置。比如这样：
...
其中有一个问题是，现在版本中，TF能接受的数据类型定义在Flink-AI-Extended的**DataTypes**中，而Flink Table字段的数据类型定义在Flink项目中的**TypeInformation**，并实现了一些基本类型如*BasicTypeInfo*和*BasicArrayTypeInfo*，但*DataTypes*和*TypeInformation*的基本类型并非一一对应。

因此，如果要将ExampleCoding配置过程封装起来，需要解决DataTypes与TypeInformation不兼容的问题，有两种方案：

1. 提供DataTypes与TypeInformation相互转换的方法。虽然两者大部分常用类型可以对应起来，但并非完全的并非一一对应，所以会存在一些无法转换的问题。
2. 抛弃DataTypes，在Flink-AI-Extended中直接采用TypeInformation。DataTypes只是一个简单的枚举类型，仅参与对数据类型的标识，采用TypeInformation也可以实现同样的功能。

方案1实现起来相对简单且容易兼容，但长远来说还是方案2较好。
...
#### \[Bug fixed][PR to issue-7] -> [pull request](https://github.com/alibaba/flink-ai-extended/pull/12) & [subsequent request](https://github.com/alibaba/flink-ai-extended/pull/16)
...
#### \[Enhancement][PR to issue-10] -> [pull request](https://github.com/alibaba/flink-ai-extended/pull/19)

**Issue (see [@link](https://github.com/alibaba/flink-ai-extended/issues/17)):**

Simplify the ExampleCoding configuration process

**Motivation:**

Now the process of configuring ExampleCoding is cumbersome. As a common configuration, we can add some tool interfaces to help users simply configure.

**Proposal (see [@link](https://github.com/alibaba/flink-ai-extended/issues/17#issuecomment-518589773)):**

Add an util class which provide simple methods to configure example coding and methods to convert DataTypes and TypeInformation.
